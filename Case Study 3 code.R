...
library(readxl) ; library(quantmod) ; library(tseries)

# Reading in data
setwd("/Users/BellaIsCute/Desktop/DA Application")
...
# Creating an empty list for each unique stock
stock_models <- lapply(unique_stocks, function(x) list()) # Might not need this?
...
for(i in 1:length(unique_stocks)) {
  ...
  stock_models[[i]] <- lm.fit # Might not need this?
...
# Looping through each stock's model and printing summary
for(i in names(stock_models)) {            #
  print(paste("Summary for stock:", i))    # Might not need this?
  print(summary(stock_models[[i]]))        #
}
stock_models
...
boxplot(All_Returns, main="Expected Return", xlab="Stock Picks", ylab="Return")
...
beta_values <- c(BetaAXP, BetaBA, BetaBAC, BetaCAT, BetaCSCO, BetaCVX, BetaDD, BetaDIS, BetaGE, BetaHD,
...
# "If a stock is riskier than the market, it will have a beta greater than 1." 
# If a stock has a beta of less than 1, the formula assumes it will reduce the
# risk of a portfolio.
...
for (i in 1:length(unique_stocks)) {
  # Subset train and test data for the current stock
  subset_train <- train[train$stock == unique_stocks[i], ]
  subset_test <- test[test$stock == unique_stocks[i], ]
  
  # Tune SVR parameters
  set.seed(123)
  tuned <- tune.svm(percent_change_next_weeks_price ~ low + volume +
                      percent_change_price + percent_change_volume_over_last_wk +
                      days_to_next_dividend + percent_return_next_dividend +
                      previous_weeks_volume, data = subset_train,
                    gamma = seq(.01, .1, by = .01),
  ...
  # Store predictions for the current stock
...
# Extract predictions for week 13 for each stock
week_13_predictions <- sapply(predictions, `[`, 13)

# Create a new dataframe for week 14
...
# Print the new dataframe for week 14
print(week_14_data)


# Merge beta values with week 14 predictions
merged_data <- merge(beta_df, week_14_data, by = "Stock")

# Plot
plot(merged_data$Beta, merged_data$Prediction,
     xlab = "Beta", ylab = "Prediction",
     main = "Beta vs. Prediction",
     pch = 19, col = "blue")

# Add labels for each point
text(merged_data$Beta, merged_data$Prediction, labels = merged_data$Stock, pos = 3, cex = 0.8)
